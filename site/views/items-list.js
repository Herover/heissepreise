...
class ItemsList extends View {
    ...
    renderItem(item) {
        ...
        elements.price.innerText = `kr ${Number(showUnitPrice ? unitPrice : price).toFixed(2)} ${priceUnit}`;
        ...
        const sparkData = [...item.priceHistory]
            ...
            .filter((e, i, org) => e.date >= 0 || (org.length > i + 1 && org[i + 1].date >= 0) || org.length == i + 1);
        // if (!item.unavailable) {
        sparkData.push({
            date: 1,
